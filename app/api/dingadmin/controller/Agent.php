...
class Agent extends Base{
    ...
    public function index(){
      $admin_auth = session('admin_auth');
        ...
        $list = Db::name('admin')->alias('a')
          ...
          ->field("a.*,b.title")->join('auth_group b','b.id=a.groupid','LEFT')->order('a.id asc')->paginate(10);
        $page = $list->render();
        return view('index',[
            'list'=>$list,
            'page'=>$page
    ...
    public function add(Request $request){
        if (request()->isPost()) {
            ...
            //判断二维码是否存在
          if(Db::name('admin')->where('code',$data["code"])->find()){
            return json(['status'=>0,'msg'=>'二维码编码已存在']);
          }
    ...
    public function edit(){
        ...
        if(request()->isPost()){
          ...
          //判断二维码是否存在
          if(Db::name('admin')->where('code',$data["code"])->where('id','<>',$data['id'])->find()){
            return json(['status'=>0,'msg'=>'二维码编码已存在']);
          }
